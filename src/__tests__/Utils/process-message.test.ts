...
import type { AuthenticationCreds, WAMessage } from '../../Types'
import { aesEncryptGCM, hmacSign } from '../../Utils/crypto'
import type { ILogger } from '../../Utils/logger'
import { cleanMessage, decryptMessageEdit, getChatId, unwrapSecretEncryptedMessage } from '../../Utils/process-message'
...
describe('secretEncryptedMessage edits', () => {
	...
	const innerEdit: proto.IMessage = {
		protocolMessage: {
	...
	const creds = { me: { id: '5513900000000:3@s.whatsapp.net', lid: '999@lid' } } as AuthenticationCreds
	...
	it('unwraps incoming edits when self credentials are not populated yet', async () => {
		const msg = makeEnvelope(sealEdit(innerEdit))
		await unwrapSecretEncryptedMessage(msg, {
	...
	it('leaves the message untouched when a MESSAGE_EDIT envelope decrypts to a non-edit protocolMessage', async () => {
		const nonEditProtocolMessage: proto.IMessage = {
			protocolMessage: {
				key: { remoteJid: authorJid, fromMe: true, id: origMsgId },
				type: proto.Message.ProtocolMessage.Type.REVOKE
			}
		}
		const msg = makeEnvelope(sealEdit(nonEditProtocolMessage))
		await unwrapSecretEncryptedMessage(msg, {
			creds,
			getMessage: async () => ({ messageContextInfo: { messageSecret: msgEncKey } }),
			logger
